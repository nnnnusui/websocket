...
class Gateway(
 ...
 ,private val logger  : Logger? = null
    ...
    private fun dispatch(payloads: Payloads) {
        ...
        val serializer = EventType.values().find { it.name == payloads.t }?.serializer ?: throw IllegalArgumentException("payloadType: ${payloads.t}: ${payloads.d}")
//        try {
            val gatewayEvent = try { json.fromJson(serializer, payloads.d) } catch (exception: Exception) { throw Exception("parseError: ${payloads.t}: ${payloads.d}") }
            client.eventManager.fire(gatewayEvent)
//        } catch (exception: MissingFieldException){
//            throw IllegalArgumentException("${payloads.t}: ${payloads.d}")
//        }
    }
    ...
    fun sendCommand(command: GatewayCommand){
        queue(command.opcode, command.toJson())
    }
}
