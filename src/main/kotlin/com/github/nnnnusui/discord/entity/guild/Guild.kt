...
import com.github.nnnnusui.discord.event.PresenceUpdate  // TODO
...
@Serializable
data class Guild(
                                                                 val lazy:                        Boolean               // TODO do not know...
 ,                                                               val id:                          Snowflake
 ,                                                               val name:                        String                // 2-100 characters
 ,                                                               val icon:                        String?               // TODO icon hash
 ,                                                               val splash:                      String?               // TODO splash hash
 ,@SerialName( "owner"                        ) @Optional val isOwner:                     Boolean?              =null
 ,@SerialName( "owner_id"                     )           val ownerId:                     Snowflake
 ,                                                     @Optional val permissions:                 Int?                  =null
 ,                                                               val region:                      String                // voice region id
 ...
 ,                                                     @Optional val members:                     Array<Guild.Member  >?=null
 ,                                                     @Optional val channels:                    Array<Channel       >?=null
 ,                                                     @Optional val presences:                   Array<PresenceUpdate>?=null // TODO partial
 ...
 ,                                                               val banner:                      String?                // banner hash
       ...
       ,ONLY_MENITIONS(1)
        ...
        companion object: KSerializer<DefaultMessageNotificationLevel> {
            ...
            override fun deserialize(decoder: Decoder): DefaultMessageNotificationLevel {
                ...
                return DefaultMessageNotificationLevel.values().firstOrNull { it.code == num } ?: throw IllegalArgumentException("invalid code `$num` (${this::class})")
            }
        }
    }
       ...
       ,ALL_MEMBERS(          2)
        ...
        companion object: KSerializer<ExplicitContentFilterLevel> {
            ...
            override fun deserialize(decoder: Decoder): ExplicitContentFilterLevel {
                val num = decoder.decodeInt()
                return ExplicitContentFilterLevel.values().firstOrNull { it.code == num } ?: throw IllegalArgumentException("invalid code `$num` (${this::class})")
            }
        }
    }
       ...
       ,ELEVATED(1)
        ...
        companion object: KSerializer<MFALevel> {
            ...
            override fun deserialize(decoder: Decoder): MFALevel {
                val num = decoder.decodeInt()
                return MFALevel.values().firstOrNull { it.code == num } ?: throw IllegalArgumentException("invalid code `$num` (${this::class})")
            }
        }
    }
       ...
       ,VERY_HIGH(4)
        ...
        companion object: KSerializer<VerificationLevel> {
            ...
            override fun deserialize(decoder: Decoder): VerificationLevel {
                val num = decoder.decodeInt()
                return VerificationLevel.values().firstOrNull { it.code == num } ?: throw IllegalArgumentException("invalid code `$num` (${this::class})")
            }
        }
    }
    @Serializable
    ...
    data class Member(
     ...
     ,                                          val roles:    Array<Snowflake> // Role object ids
     ,@Serializable(with = ISO8601Serializer::class)
