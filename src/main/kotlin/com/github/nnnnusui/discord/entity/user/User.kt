...
data class User(
 ...
 ,@SerialName("username"     )@Optional  val username:      String?           =null  // not unique
 ,@SerialName("discriminator")@Optional  val discriminator: String?           =null  // TODO 4-digit discord-tag
 ,@SerialName("avatar"       )@Optional  val avatar:        String?           = null // nullable // TODO avatar hash
 ,@SerialName("bot"          ) @Optional val isBot:         Boolean?          =null
 ,@SerialName("mfa_enabled"  ) @Optional val isMfaEnabled:  Boolean?          =null
 ,@SerialName("locale"       ) @Optional val locale:        String?           =null
 ,@SerialName("verified"     ) @Optional val isVerified:    Boolean?          =null
 ,@SerialName("email"        ) @Optional val email:         String?           =null
 ,@SerialName("flags"        ) @Optional val flags:         User.Flag?        =null
 ,@SerialName("premium_type" ) @Optional val premiumType:   User.PremiumType? =null
){
    @Serializable(with = Flag.Companion::class)
    ...
    @Serializable(with = PremiumType.Companion::class)
